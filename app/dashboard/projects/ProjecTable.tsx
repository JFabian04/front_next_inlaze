    ...
    const showTasks = (item: Project) => {
        if (item) {
            ...
            if (userId) {
                ...
                params.set('item', item.name?.toString());
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {currentItems.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <div
                                        className={`flex items-center gap-5 px-5 py-2 shadow bg-gray-200/50 rounded-full ${authData.rol === 'admin' ? 'justify-between' : 'justify-center'
                                            }`}
                                    >
                                        {/* Botón de Actualizar */}
                                        {authData && authData.rol === 'admin' && (
